...
$pdf->Image('../../style/assets/img/logo4.png',15,18,22);
$pdf->Image('../../style/assets/img/logo.png',255,18,24);
$pdf->SetTitle("Cetak Data");
$pdf->SetFont('Arial','B',16);
$pdf->Cell(0,5,'DINAS PENDIDIKAN PROVINSI JAMBI','0','1','C',false);
$pdf->SetFont('Arial','B',18);
$pdf->Cell(0,5,'SEKOLAH MENENGAH ATAS (SMA) NEGERI 11','0','1','C',false);
$pdf->Cell(0,5,'KOTA JAMBI','0','1','C',false);
$pdf->SetFont('Arial','i',8);
$pdf->Cell(0,5,'Alamat : Jln. Sersan Anwar Bay Kel. Bagan Pete Kec. Alam Barajo','0','1','C',false);
$pdf->Cell(0,2,'Telp : (0741) 583044 - Website : www.sma11kotajambi.sch.id','0','1','C',false);
$pdf->Ln(3);
...
if(@$_GET['data'] == "pengajar") {
	...
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(50,5,'Laporan Daftar Kelas, Letak Ruang, Wali dan Ketua Masing-masing Kelas di SMA Negeri 11 Kota Jambi','0','1','L',false);
	$pdf->Ln(3);
	...
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(50,5,'Laporan Data Siswa Beserta Nilainya yang Mengikuti Tugas / Quiz','0','1','L',false);
	...
	while($data_siswa_mengikuti_tes = mysqli_fetch_array($sql_siswa_mengikuti_tes)) {
		...
		$pdf->Cell(15,4,$data_siswa_mengikuti_tes['nama_kelas'],1,0,'C');
            ...
            if(is_numeric($nilai_essay)) {
            	$output = "Soal Pilgan : [".$data_pilgan['presentase']."]. Soal Essay : [".$nilai_essay."]. Nilai total : [". ($data_pilgan['presentase'] + $data_essay['nilai']) / 2 ."]";
            } else if($nilai_essay == 'Belum dikoreksi') {
            	$output = "Soal Pilgan : [".$data_pilgan['presentase']."]. Soal Essay : [".$nilai_essay."]. Nilai total : [ Menunggu soal essay dikoreksi]";
            } else if($nilai_essay == 'Tidak ada soal essay') {
            	$output = "Soal Pilgan : [".$data_pilgan['presentase']."]. Soal Essay : [".$nilai_essay."]. Nilai total : [".$data_pilgan['presentase']."]";
            }
	...
	$pdf->Cell(50,6,'Judul Tugas / Quiz',1,0,'C');
	...
	while($data_nilai_pilgan = mysqli_fetch_array($sql_nilai_pilgan)) {
		...
		if(mysqli_num_rows($sql_cek_jawaban) > 0) {
			...
			$pdf->Cell(65,6,'Tugas / Quiz ini tidak ada soal essay',1,0,'C');
...
$pdf->Cell(0,0,"Jambi, ".tgl_indo(date('Y-m-d')),0,0,'L');
